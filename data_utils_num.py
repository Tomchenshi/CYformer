...
def load_img(filepath):
    ...
    x = x['msi'].astype(np.float32)  #cave
    #x = x['hsi'].astype(np.float32)  # harvard
...
def load_img1(filepath):
    x = scio.loadmat(filepath)
    x = x['RGB'].astype(np.float32)  # cave
    #x = x['msi'].astype(np.float32)   # harvard
    x = torch.tensor(x).float()
    return x
